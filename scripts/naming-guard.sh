...
# names, or standard CLI vocabulary such as `Command`. The softer cases in docs/domain-language.md
...
if grep -rnE "\b(struct|enum|trait|type|mod)[[:space:]]+[A-Za-z0-9_]*(${banned})[A-Za-z0-9_]*" --include='*.rs' crates/; then
    echo "" >&2
    echo "naming-guard: a queue-runtime/CQRS word names a ringi type/module — see docs/domain-language.md." >&2
