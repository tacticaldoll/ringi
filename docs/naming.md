...
`Arbitrator`, `Evaluator`, `Dissent`, `Risk`, `Question`, `Condition`, `Move`, `Decision`, and
`Archive`. The CLI's `Command` is standard CLI vocabulary, not a domain concept — it is fine and
...
suunta is navigation (`Bearing`, `Course`, `Sigil`), cadw is batch-fold (`TargetId`, `Ledger`,
`Move`, `Validator`, `Rejection`). When ringi calls a brick, it uses that brick's term — **but
only in thin seam adapters**.
...
the hard mechanics are the bricks' (durable lifecycle → pacta, convergence → suunta, atomic
batch-fold validation → cadw). Naming one of them yourself is the monolith returning.
